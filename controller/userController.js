...
exports.updatePassword = catchAsync(async (req, res, next) => {
  const currentUser = await User.findById(req.body.id).select('+password');
  ...
  // 3) check if Posted possword is currect?

  const currectPassword = await currentUser.correctPassword(
  ...
  if (!currectPassword)
    return next(
      new AppError('your current password not match! please Try again', 401)
    );

  currentUser.password = req.body.password;
  currentUser.passwordConfirm = req.body.passwordConfirm;
  await currentUser.save();
  createJWT(currentUser, 200, res);
});
